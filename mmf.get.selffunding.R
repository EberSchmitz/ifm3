...
#' 
#' ex.mmf.selffunding <- mmf.get.selffunding(ex.mmf[['cfs.discounted']])
#' 
mmf.get.selffunding <- function(mmf.cfs) {
  all.selffunding <- list()
  for (count in 1:length(mmf.cfs)) {
        for (i in 1:length(mmf.cfs[[count]])){
           acValue <- mmf.cfs[[count]][i]
           NextValue <- mmf.cfs[[count]][i+1]              
           if (NextValue > acValue) {
             all.selffunding[[count]] <- i+1
             break
           }
        }  
  }
...
mmf.get.min.selffunding <- function (mmf.get.selffunding){
  return(which.min(mmf.get.selffunding))
}